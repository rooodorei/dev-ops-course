pipeline {
    ...
    stages {
        ...
        stage('备份目标文件夹下文件'){
            steps {
                bat '''move "C:\\workspace\\jars\\*" "C:\\workspace\\jars\\backup"
                       exit 0'''
            }
        }
        ...
        stage('复制打包文件到目标文件夹'){
            steps {
                bat '''robocopy register-center-svc\\target C:\\workspace\\jars register-center-svc-0.0.1-SNAPSHOT.jar
    ...
    post {
        success {
            script {
                echo '启动项目'
                bat 'call C:\\workspace\\start.bat'
            }
        }
    }
}